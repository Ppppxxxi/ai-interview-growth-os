...
当前项目可以作为 v0.2 Personal MVP 发布。风险重点不是代码安全，而是公开仓库里的样例数据、浏览器本地数据截图、过程文档和产品能力边界是否容易被误读。
...
- `src/domain/sampleData.ts` 中的公司、岗位和对话是虚构样例，应在 README 中保持“样例数据 / mock agent”表述。
...
- 如果目标是让仓库更轻量，可以后续把过程文档移到产品案例文档，只保留 README、Roadmap 和 Agent 合约。
